...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    #fPrevia { font-size: 1.6rem; font-weight: 900; letter-spacing: 3px; color: #b8a120; transition: color .2s; }
...
<body>
  ...
  <div class="topbar">
    <div class="topbar-inner">
      <div class="topbar-title">
        <div class="t-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></div>
        Nueva Festividad
      </div>
  ...
  <div class="page-wrap">
    <div class="form-card">
      ...
      <div class="form-card-header">
        <div class="fch-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></div>
        <h2>Nueva Festividad</h2>
        <p>Configura el estilo festivo que aparecerá en el header del sitio.</p>
      </div>
        ...
        <form action="{{ route('festividades.store') }}" method="POST">
          ...
          <div class="grid2">
            <div class="field-group">
              ...
              <input type="text" name="nombre" id="nombre" class="zfield"
                     placeholder="Ej: Navidad, Día de Muertos..." value="{{ old('nombre') }}" required>
            </div>
            <div class="field-group">
              <label class="field-label" for="texto_header">Texto en el header <span class="req">*</span></label>
              <input type="text" name="texto_header" id="texto_header" class="zfield"
                     placeholder="Ej: ZARMEX ⛄" maxlength="50" value="{{ old('texto_header', 'ZARMEX') }}" required>
          ...
          <div class="grid2">
            <div class="field-group">
              ...
              <div class="color-row">
                <input type="color" name="color_texto" id="colorPicker" class="color-picker-input" value="{{ old('color_texto', '#b8a120') }}">
                @foreach(['#b8a120'=>'Dorado','#c0392b'=>'Rojo','#27ae60'=>'Verde','#e67e22'=>'Naranja','#8e44ad'=>'Morado','#e91e8c'=>'Rosa','#2980b9'=>'Azul'] as $hex => $label)
                  <div class="color-swatch {{ old('color_texto','#b8a120') === $hex ? 'sel' : '' }}"
                       style="background:{{ $hex }};" data-color="{{ $hex }}" title="{{ $label }}"></div>
                @endforeach
              </div>
            </div>
            <div class="field-group">
              ...
              <div class="sec-select-wrap">
                <select name="efecto" id="efecto" class="zfield zselect">
                  <option value="none"     {{ old('efecto','none')==='none'     ? 'selected':'' }}>Sin efecto</option>
                  <option value="glow"     {{ old('efecto')==='glow'            ? 'selected':'' }}>✨ Brillo (glow)</option>
                  <option value="tricolor" {{ old('efecto')==='tricolor'        ? 'selected':'' }}>🇲🇽 Tricolor</option>
                  <option value="shadow"   {{ old('efecto')==='shadow'          ? 'selected':'' }}>🌑 Sombra oscura</option>
                  <option value="outline"   {{ old('efecto')==='outline'   ? 'selected':'' }}>✏️ Contorno</option>
                  <option value="rainbow"   {{ old('efecto')==='rainbow'   ? 'selected':'' }}>🌈 Arcoíris</option>
                  <option value="fire"      {{ old('efecto')==='fire'      ? 'selected':'' }}>🔥 Efecto fuego</option>
                  <option value="ice"       {{ old('efecto')==='ice'       ? 'selected':'' }}>❄️ Efecto hielo</option>
                  <option value="neon"      {{ old('efecto')==='neon'      ? 'selected':'' }}>💡 Neón</option>
                  <option value="vintage"   {{ old('efecto')==='vintage'   ? 'selected':'' }}>🎞️ Vintage</option>
                  <option value="pulse"     {{ old('efecto')==='pulse'     ? 'selected':'' }}>💓 Pulso</option>
                  <option value="shimmer"   {{ old('efecto')==='shimmer'   ? 'selected':'' }}>✨ Destello</option>
                  <option value="retro"     {{ old('efecto')==='retro'     ? 'selected':'' }}>📺 Retro</option>
                  <option value="matrix"    {{ old('efecto')==='matrix'    ? 'selected':'' }}>🟩 Matrix</option>
                  <option value="pirate"    {{ old('efecto')==='pirate'    ? 'selected':'' }}>☠️ Pirata</option>
                  <option value="carnival"  {{ old('efecto')==='carnival'  ? 'selected':'' }}>🎡 Carnaval</option>
                  <option value="coral"     {{ old('efecto')==='coral'     ? 'selected':'' }}>🪸 Coral</option>
                  <option value="aurora"    {{ old('efecto')==='aurora'    ? 'selected':'' }}>🌌 Aurora boreal</option>
                  <option value="lava"      {{ old('efecto')==='lava'      ? 'selected':'' }}>🌋 Lava</option>
                </select>
              </div>
            </div>
          </div>
            ...
            <div class="deco-grid">
              @foreach(['nieve'=>'❄️ Nieve','flores'=>'🌸 Flores','velas'=>'🕯️ Velas','murcielagos'=>'🦇 Murciélagos','fantasmas'=>'👻 Fantasmas','calabazas'=>'🎃 Calabazas','corazones'=>'❤️ Corazones','confetti'=>'🎊 Confetti','estrellas'=>'⭐ Estrellas','rosas'=>'🌹 Rosas','acebo'=>'🍃 Acebo','banderas'=>'🇲🇽 Banderas','fuegos'=>'🎆 Fuegos artificiales','arboles'=>'🎄 Árbol de Navidad','regalos'=>'🎁 Regalos','campanas'=>'🔔 Campanas','globos'=>'🎈 Globos','soles'=>'☀️ Soles','lunas'=>'🌙 Lunas','arcoiris'=>'🌈 Arcoíris','mariposas'=>'🦋 Mariposas','diamantes'=>'💎 Diamantes','coronas'=>'👑 Coronas','notas'=>'🎵 Notas musicales','serpentinas'=>'🎊 Serpentinas','brujitas'=>'🧙 Brujitas'] as $val => $label)
                <label class="deco-chip">
                  <input type="checkbox" name="decoraciones[]" value="{{ $val }}"
                         {{ in_array($val, old('decoraciones', [])) ? 'checked' : '' }}>
                  {{ $label }}
                </label>
              @endforeach
            </div>
          ...
          <div class="grid2">
            <div class="field-group">
              ...
              <input type="date" name="fecha_inicio" id="fecha_inicio" class="zfield" value="{{ old('fecha_inicio') }}">
            </div>
            <div class="field-group">
              <label class="field-label" for="fecha_fin">Fin automático <span style="font-weight:400;color:#9ca3af;">(opcional)</span></label>
              <input type="date" name="fecha_fin" id="fecha_fin" class="zfield" value="{{ old('fecha_fin') }}">
            </div>
          </div>
          <div class="field-hint" style="margin-top:-10px;margin-bottom:16px;">
            Sin fechas, se activa manualmente. Con fechas, el sistema la activa y desactiva solo.
          </div>
          ...
          <div class="field-group">
            ...
            <div class="header-preview">
              ...
              <span id="fPrevia">{{ old('texto_header', 'ZARMEX') }}</span>
            </div>
          </div>
  ...
  <script>
    ...
    function applyPreview() {
      ...
      const efecto = efectoSel.value;
      previa.style.cssText = `font-size:1.6rem;font-weight:900;letter-spacing:3px;transition:color .2s;`;
      if (efecto === 'glow') {
        previa.style.color = color;
        previa.style.textShadow = `0 0 10px ${color}99`;
      } else if (efecto === 'shadow') {
        previa.style.color = color;
        previa.style.textShadow = '2px 2px 6px rgba(0,0,0,.8)';
      } else if (efecto === 'tricolor') {
        previa.style.background = 'linear-gradient(90deg,#006847 33%,#fff 33%,#fff 66%,#ce1126 66%)';
        previa.style.webkitBackgroundClip = 'text';
        previa.style.webkitTextFillColor = 'transparent';
        previa.style.backgroundClip = 'text';
      } else {
        previa.style.color = color;
      }
    }
