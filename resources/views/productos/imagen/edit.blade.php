...
<head>
    ...
    <section class="container mt-5">
        <div class="form-container">
                <div class="form-container">
                <h2>Editar Imagen de <br>{{ $imagen->producto->nombre }}</h2>
                <div class="form-group d-flex justify-content-between mb-3">
                    <a class="btn btn-secondary btn-sm" href="{{ route('productos.imagenes.show', $imagen->producto_id) }}">Regresar</a>
                </div>
                <form action="{{ route('productos.imagenes.update', $imagen->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
        
                    <div class="form-group">
                        <label for="id">ID de Imagen:</label>
                        <input type="text" id="id" name="id" value="{{ $imagen->id }}" readonly>
                    </div>
        
                    <div class="form-group">
                        <label>Imagen Actual:</label>
                        <img src="{{ asset($imagen->ruta) }}" alt="Imagen {{ $imagen->producto->nombre }}" width="100">
                    </div>
